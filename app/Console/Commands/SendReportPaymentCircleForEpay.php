...
class SendReportPaymentCircleForEpay extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'SendReportPaymentCircleForEpay {--type=}';
    ...
    public function handle(): void
    {
        ...
        try {
            if ($type === 'weekly') {
                ...
                $deliveryReport = EpayReportType::EVERY_PAYMENT_CIRCLE->value;
                $paymentCycle = MerchantStorePaymentCycle::WEEKEND->value;
            } elseif ($type === 'monthly') {
                $periodFrom = Carbon::now()->subMonth()->startOfMonth();
                $periodTo = Carbon::now()->subMonth()->endOfMonth();
                $deliveryReport = EpayReportType::EVERY_PAYMENT_CIRCLE->value;
                $paymentCycle = MerchantStorePaymentCycle::MONTH_END->value;
            } else {
                $periodFrom = Carbon::now()->startOfDay()->subDays(3);
                $periodTo = Carbon::now()->endOfDay()->subDays(1);
                $deliveryReport = EpayReportType::EVERY_PAYMENT_CIRCLE->value;
                $paymentCycle = MerchantStorePaymentCycle::THREE_DAYS_END->value;
            }
            ...
            $merchantStoreDatas = $merchantStoreRepository
                ...
                ->where('payment_cycle', $paymentCycle)
                ->get();

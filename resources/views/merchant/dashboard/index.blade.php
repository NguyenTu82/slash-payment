@extends('merchant.layouts.base', ['title' => __('admin.titles')])
@section('title', __('merchant.admin.dashboard_title'))
                    ...
                    <div class="card-body">
                        <form action="" method="GET" id="form-seach" class="">
                            <div class="row row-select">
                                ...
                                <!-- select merchant-->
                                <h6 class="text-primary merchant-select">
                                    {{ __('common.dashboard.aggregated_merchant') }}
                                </h6>
                                <div class="select_search" id="day-box">
                                    <select class="select_list select_option" name="merchant_slt">
                                        <option value="">{{ __('common.dashboard.default') }}
                                        </option>
            ...
            <div class="col-12">
                <div class="card card-mer-dash-table">
                    ...
                    <div class="table-responsive p-0">
                        ...
                        @include('merchant.dashboard.modal.detail')
...
@push('script')
    <script>
        ...
        function getDetailDashboard(element = false, formData = false) {
            ...
            $.ajax({
                url: "{{ route('merchant.dashboard.detail.get') }}",
                type: 'GET',
                headers: {
        ...
        function getDataDashboard(formData) {
            $.ajax({
                url: "{{ route('merchant.dashboard.index.get') }}",
                type: 'GET',
                headers: {
                    'Accept': 'application/json'
                },
        ...
        function exportCSV() {
            $(TABLE_STATISTIC).table2csv({
                filename: 'epay_dashboard_' + Date.now() + '.csv',
                separator: ',',
                newline: '\n',
                quoteFields: true,

...
@section('title', __('common.sidebar.payment_request'))
    ...
    <div class="row">
        <div class="col-12 detail-list-manager">
            <div class="account bg-white page-white">
                <form class="form-detail form-dropdown-input form-validate d-flex" id="create-request-withdraw-form">
                    @csrf
                    <div class='left-wrapper'>
                        ...
                        <input type="hidden" name="auth_submit" value="epay">
                        ...
                        <div class="form-group row form-item mg-top-first-form ">
                            ...
                            <div class="col col-md-10">
                                <div class="form-edit-input">
                                    @if (count($allMerchantStores) > 0)
                                        <input type="text" class="form-control form-control-w337" id="merchant_store_id"
                                            name="merchant_store_input"
                                            value="{{ formatAccountId($allMerchantStores[0]->merchant_code) }} - {{ $allMerchantStores[0]->name }}">
                                        <input type="hidden" name="merchant_store_id"
                                            value="{{ $allMerchantStores[0]->id }}">
                                    @else
                        ...
                        <div class="form-group row form-item ">
                            ...
                            <div class="col col-md-10">
                                <input name="company_member_code" value="{{ old('company_member_code') }}" type="text"
                                    class="form-control">
                            </div>
                        </div>
                                    ...
                                    <div class="form-input-mini-item">
                                        ...
                                        <div class="form-noti-request">
                                            ...
                                            <input name="amount" value="{{ old('amount') }}" type="number"
                                                class="form-control form-control-w293">
                                        </div>
                                    </div>
                                </div>
                                <div class="note-pass"></div>
...
@push('script')
    <script>
        ...
        const ALL_MERCHANT_STORES = @json($allMerchantStores);
        ...
        function validateBankingPayment() {
            ...
            // $(FINANCIAL_INSTITUTION_NAME_INPUT).rules('add', {
            //     required: true,
            //     messages: {
            //         required: "{{ __('merchant.withdraw.validation.financial_institution_name.required') }}",
            //     }
            // })
            $(BRANCH_CODE_INPUT).rules('add', {
                checkBranchCodeRule: true
            })
            // $(BRANCH_NAME_INPUT).rules('add', {
            //     required: true,
            //     messages: {
            //         required: "{{ __('merchant.withdraw.validation.branch_name.required') }}",
            //     }
            // })
        ...
        async function checkBankCode() {
            ...
            return $.ajax({
                ...
                console.log(response, 'responseresponse', BANK_CODE_INPUT.attr('id') + '-error',
                    FINANCIAL_INSTITUTION_NAME_INPUT.attr('id') + '-error')
                if (response.data) {
                    console.log(111)
        ...
        function getFiatAccountDefault() {
            ...
            let url = '{{ route('admin_epay.withdraw.fiat-account-default.get', ':merchantStoreId') }}';
        ...
        function getCryptoAccountDefault() {
            let merchantStoreId = $(MERCHANT_STORE_SLT).val();
            let url = '{{ route('admin_epay.withdraw.crypto-account-default.get', ':merchantStoreId') }}';
            url = url.replace(':merchantStoreId', merchantStoreId);
        ...
        function getInfoStoreSelected() {
            let merchantStoreId = $(MERCHANT_STORE_SLT).val();
            let store = ALL_MERCHANT_STORES.find(item => {
        ...
        $(document).ready(function() {
            ...
            SUBMIT_BUTTON_COMMON.on('click', function() {
                ...
                let url = '{{ route('admin_epay.withdraw.request.store.post') }}';
                ...
                $.ajax({
                    ...
                    if (response.status) {
                        window.location.href =
                            "{{ route('admin_epay.withdraw.history.index.get') }}"
                    } else {
            ...
            CANCEL_CREATE_BTN.on('click', function() {
                window.location.href = `{{ route('admin_epay.withdraw.history.index.get') }}`;
            });
